...
!               / (1.0 + neutral_amp * var)
    ...
    if( sys3hot ) then
      elecHot_multiplier=elecHot_multiplier+sys3_amp*(cos((290-longitude)*dTOr))
    endif
...
!      v_ion=0.0+(((mass_loading(mype+1)*volume*LNG_GRID)*57.0*(rdist)**5)/(0.8*sqrt(1.0-(1.0/(rdist)))*4.0*PI*1.5*((Rj*1.0e3)**2)*(4.2e-4)**2))
      ...
      if( fehnl2) then
          call MPI_ALLREDUCE(gradnl2global, all_grad, NUMPES, MPI_REAL, MPI_SUM, MPI_COMM_WORLD, ierr)
          do k = (radgrid-1)*LNG_GRID+1, radgrid*LNG_GRID
             avgrad = avgrad + all_grad(k)
          end do
          avgrad = avgrad/LNG_GRID
          n%fh = n%fh*(gradnl2global(mype)/avgrad)
          if (all_grad(mype) .ne. gradnl2global(mype)) print*, "Allgrad not equal to global Nl2 gradient"
      end if
end if
...
!                 if( rdist < reac_off_dist ) call OtherOutput3D(200.0*mass_loading(mype+1)/ave_loading, longitude+((j+1)/(LNG_GRID+1))*360.0, rdist, day_char, 'LOAD')
              ...
              if(OUTPUT_INTS) then !Intensity
               ...
               ! intensity= n%sp*T_ave/(n_ave*T%sp)
                call IonOutput3D(n%sp/T%sp, n%s2p/T%s2p, n%s3p/T%s3p, n%op/T%op, n%o2p/T%o2p, &
                  longitude+((j+1)/(LNG_GRID+1))*360.0, rdist, day_char, 'INTS')
                if( j .eq. LNG_GRID ) call spacer3D(day_char, 'INTS')
...
! Added this here in hope to change Temp transport
do i=1, aztrans_it
    call az_transport(n, nrg)
  enddo
